...
app.filter('get_date_agenda_numbers', function() { return function(begin,end,trans) {
    ...
    if(begin==end){
        ...
        string += bg_arr[2]+'.'+bg_arr[1]+'&ndash;'+en_arr[2]+'.'+en_arr[1];
        //string += bg_arr[2]+'.'+bg_arr[1]+'&ndash;'+en_arr[2]+'.'+en_arr[1];
...
app.controller('FilterCategoryController', ['$scope', '$http', '$log', '$q', '$location', '$window', '$filter', '$timeout','$rootScope', 'AppService', 'DateService', function($scope, $http, $log, $q, $location, $window, $filter, $timeout,$rootScope,AppService,DateService) {
    ...
    $scope.getFiltersResult = function(){
        ...
        $http.post(AppService.url+'/api/get/filter/advanced/agenda/result', {
                filters:cats,
                year:$scope.data.selected_year,
                current_month:$scope.data.current_month,
                is_all:$scope.data.all,
                all_filters:$scope.data.filters
                })
            .then(
                function successCallback(response){
    ...
    $scope.changeMonthByClick = function(month,$index){
        ...
        if($scope.data.filter_months[0].nr_month == 12 && month.nr_month == 1){
            $scope.changeYearLine('next');
            //$scope.data.selected_year ++;
            //$scope.data.current_year ++;
            console.log("styczeń!!!")
        }else if($scope.data.filter_months[1].nr_month == 1 && month.nr_month == 12){
            $scope.changeYearLine('prev');
        }
